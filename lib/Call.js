...
// Added in for multi-process capability if redis is available
var redis;
...
Call.prototype._setCb = function (handler, callback) {
	if (redis) {
		redis.incr("twilio-api", function (err, reply) {
			var myID = reply;
			redis.set("twilio-api:"+myID, handler.toString(), function () {
				callback(myID);
			});
		});
	} else {
		var myID = this._cbID;
		this._cbID++;
		this._cb[myID] = handler;
		callback(myID);
	}
}

Call.prototype._getCb = function (cbId, callback) {
	if (redis) {
		redis.get("twilio-api:"+cbId, function (err, reply) {
			eval("var cb = "+reply);
			callback(cb);
		});
	} else {
		callback(this._cb[cbId]);
	}
}
...
Call.prototype.liveCb = function(cb) {
	this._setCb(cb, function (cbid) {
		this.liveRedirect(this._app.VoiceUrl + "?cbtype=liveCb&cb=" + encodeURIComponent(cbid),
...
Call.prototype.cb = function(cb) {
	this._setCb(cb, function (cbid) {
		this.redirect(this._app.VoiceUrl + "?cbtype=cb&cb=" +
			encodeURIComponent(cbid), this._app.VoiceMethod);
	});
}
/* TwiML <Say> Verb */
